...
QB.Phone.Functions.DoBankOpen = function() {
    QB.Phone.Data.PlayerData.money.bank = QB.Phone.Data.PlayerData.money.bank.toFixed(0);
    ...
    if (iban != "" && amount != "") {
        $.post('https://qb-phone/CanTransferMoney', JSON.stringify({
            ...
            if (data.TransferedMoney) {
                ...
                QB.Phone.Notifications.Add("fas fa-university", "Bank", "You have transferred &dollar; " + amount + "!", "#badc58", 1500);
            } else {
...
$(document).on('click', '.decline-invoice', function(event) {
    ...
    QB.Phone.Notifications.Add("fas fa-university", "Bank", "You have declined to pay &dollar;" + InvoiceData.amount + "!", "#badc58", 1500);
});
...
QB.Phone.Functions.LoadBankInvoices = function(invoices) {
    if (invoices !== null) {
        ...
        $.each(invoices, function(i, invoice) {
            var Elem = '<div class="bank-app-invoice" id="invoiceid-' + i + '"> <div class="bank-app-invoice-title">' + invoice.title + ' </div> <div class="bank-app-invoice-amount">&dollar; ' + invoice.amount + '<span style="font-size: 1vh; color: gray;">  (Payable: ' + QB.Phone.Functions.CamelCase(invoice.society) + ')</span></div> <div class="bank-app-invoice-buttons"> <i class="fas fa-check-circle pay-invoice"></i> <i class="fas fa-times-circle decline-invoice"></i> </div> </div>';
...
QB.Phone.Functions.LoadBankPaychecks = function(paychecks) {
    if (paychecks !== null) {
        //console.log(paychecks)
        $(".bank-app-paychecks-list").html("");

        $.each(paychecks, function(i, paycheck) {
            //console.log(paycheck.date)
            var date = QB.Phone.Functions.FormatDate(paycheck.date)
            var Elem = '<div class="bank-app-paycheck" id="paycheckid-' + i + '"> <div class="bank-app-paycheck-title">' + paycheck.title + ' </div> <div class="bank-app-paycheck-amount">&dollar; ' + paycheck.amount + '</div> <div class="bank-app-paycheck-date">' + date + ' </div> </div>';

            $(".bank-app-paychecks-list").append(Elem);
            //$("#paycheckid-" + i).data('paycheckdata', paycheck);
        });
    }
}

QB.Phone.Functions.CamelCase = function(str) {
    return str.replace(
        /\w\S*/g,
        function(txt) {
...
$(document).on('click', '.bank-app-my-contact', function(e) {
    ...
    if (PressedContactData.iban !== "" && PressedContactData.iban !== undefined && PressedContactData.iban !== null) {
        ...
        QB.Phone.Notifications.Add("fas fa-university", "Bank", "No IBAN associated with this contact!", "#badc58", 2500);
    }
    QB.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});

QB.Phone.Functions.FormatDate = function(value) {
