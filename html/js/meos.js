...
$(document).on('click', '.person-search-result', function(e) {
    ...
    var OpenElement = '<div class="person-search-result-name">Name: ' + ClickedPersonData.firstname + ' ' + ClickedPersonData.lastname + '</div> <div class="person-search-result-bsn">Citizen Id: ' + ClickedPersonData.citizenid + '</div> <div class="person-opensplit"></div> &nbsp; <div class="person-search-result-dob">Birthdate: ' + ClickedPersonData.birthdate + '</div> <div class="person-search-result-number">Telephone Number: ' + ClickedPersonData.phone + '</div> <div class="person-search-result-nationality">Nationality: ' + ClickedPersonData.nationality + '</div> <div class="person-search-result-gender">Gender: ' + Gender + '</div> &nbsp; <div class="person-search-result-apartment"><span id="' + ClickedPersonId + '">Apartment: ' + appartementData.label + '</span> <i class="fas fa-map-marker-alt appartment-adress-location" id="' + ClickedPersonId + '"></i></div> &nbsp; <div class="person-search-result-warned">Flagged: ' + IsWarrant + '</div> <div class="person-search-result-driverslicense">Drivers license: ' + HasLicense + '</div>';
    ...
    if (OpenedPerson === null) {
        ...
        var PreviousElement = '<div class="person-search-result-name">Name: ' + PreviousPersonData.firstname + ' ' + PreviousPersonData.lastname + '</div> <div class="person-search-result-bsn">Citizen Id: ' + PreviousPersonData.citizenid + '</div>';
...
$(document).on('click', '.person-search-result-apartment > span', function(e) {
    ...
    QB.Phone.Notifications.Add("fas fa-university", "MEOS", "House number copied!", "#badc58", 1750);
...
$(document).on('click', '.person-search-result-house', function(e) {
    ...
    var OpenElement = '<div class="person-search-result-name">Owner: ' + ClickedHouseData.charinfo.firstname + ' ' + ClickedHouseData.charinfo.lastname + '</div><div class="person-search-result-bsn">House: ' + ClickedHouseData.label + '</div> <div class="person-opensplit"></div> &nbsp; <div class="person-search-result-dob">Adres: ' + ClickedHouseData.label + ' &nbsp; <i class="fas fa-map-marker-alt house-adress-location" id="' + ClickedHouseId + '"></i></div> <div class="person-search-result-number">Tier: ' + ClickedHouseData.tier + '</div> <div class="person-search-result-nationality">Garage: ' + GarageLabel + '</div>';
        ...
        $.post('https://qb-phone/FetchSearchResults', JSON.stringify({
            ...
            if (result != null) {
                ...
                $.each(result, function(i, person) {
                    var PersonElement = '<div class="person-search-result" id="person-' + i + '"><div class="person-search-result-name">Name: ' + person.firstname + ' ' + person.lastname + '</div> <div class="person-search-result-bsn">Citizen Id: ' + person.citizenid + '</div> </div>';
...
$(document).on('click', '.scan-search-vehicle', function(e) {
    ...
    $.post('https://qb-phone/FetchVehicleScan', JSON.stringify({}), function(vehicle) {
        if (vehicle != null) {
            ...
            var VehicleElement = '<div class="vehicle-search-result"> <div class="vehicle-search-result-name">' + vehicle.label + '</div> <div class="vehicle-search-result-plate">Badge: ' + vehicle.plate + '</div> <div class="vehicle-opensplit"></div> &nbsp; <div class="vehicle-search-result-owner">Owner: ' + vehicle.owner + '</div> &nbsp; <div class="vehicle-search-result-apk">APK: ' + APK + '</div> <div class="vehicle-search-result-warrant">Flagged: ' + Flagged + '</div> </div>'
            $(".vehicle-search-results").append(VehicleElement);
        } else {
...
AddPoliceAlert = function(data) {
    ...
    if (data.alert.coords != undefined && data.alert.coords != null) {
        AlertElement = '<div class="meos-alert" id="alert-' + randId + '"> <span class="meos-alert-new" style="margin-bottom: 1vh;">NEW</span> <p class="meos-alert-type">Melding: ' + data.alert.title + '</p> <p class="meos-alert-description">' + data.alert.description + '</p> <hr> <div class="meos-location-button">LOCATION</div> </div>';
    } else {
        AlertElement = '<div class="meos-alert" id="alert-' + randId + '"> <span class="meos-alert-new" style="margin-bottom: 1vh;">NEW</span> <p class="meos-alert-type">Melding: ' + data.alert.title + '</p> <p class="meos-alert-description">' + data.alert.description + '</p></div>';
    }
    ...
    if (data.alert.title == "Assistant colleague") {
...
$(document).on('click', '.meos-recent-alert', function(e) {
    ...
    if (alertData.coords != undefined && alertData.coords != null) {
        ...
        QB.Phone.Notifications.Add("politie", "MEOS", "This alert does not have GPS Location!");
    }
});
