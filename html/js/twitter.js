...
QB.Phone.Notifications.LoadTweets = function(Tweets) {
    ...
    if (Tweets !== null && Tweets !== undefined && Tweets !== "" && Tweets.length > 0) {
        ...
        $.each(Tweets, function(i, Tweet) {
            ...
            var PictureUrl = "./img/default.webp"
    ...
    if (Tweets.length > 0) {
        ...
        $.each(Tweets, function(i, Tweet) {
            ...
            if ( /(https?:\/\/)?([\da-z\.-]+)\.([a-z]{2,6})([\/\w\.-]*)*\/?/.test(text) ) {
                text = '<img src=' + text + ' width="200px" height="auto"/>'
            };
...
$(document).on('click', '#send-tweet', function(e) {
    ...
    if (TweetMessage != "") {
        ...
        $.post('https://qb-phone/PostNewTweet', JSON.stringify({
            Message: embedImage(TweetMessage),
            Date: CurrentDate,
    ...
    if (hashtags !== null) {
        ...
        $.each(hashtags, function(i, hashtag) {
            ...
            if (hashtag.messages.length >= MinimumTrending) {
                Elem = '<div class="twitter-hashtag" id="tag-' + hashtag.hashtag + '"><div class="twitter-hashtag-status">Trending in Twitter</div> <div class="twitter-hashtag-tag">#' + hashtag.hashtag + '</div> <div class="twitter-hashtag-messages">' + hashtag.messages.length + ' ' + TweetHandle + '</div> </div>';
            } else {
                Elem = '<div class="twitter-hashtag" id="tag-' + hashtag.hashtag + '"><div class="twitter-hashtag-status">Now trending in Twitter</div> <div class="twitter-hashtag-tag">#' + hashtag.hashtag + '</div> <div class="twitter-hashtag-messages">' + hashtag.messages.length + ' ' + TweetHandle + '</div> </div>';
            }
...
function embedImage(text) {
    //console.log(text);
    //console.log(/(https?:\/\/)?([\da-z\.-]+)\.([a-z]{2,6})([\/\w\.-]*)*\/?/.test(text));
    if ( /(https?:\/\/)?([\da-z\.-]+)\.([a-z]{2,6})([\/\w\.-]*)*\/?/.test(text) ) {
        text = '<img src=' + text + ' width="200px" height="auto"/>'
    };
    return text
...
/* $(document).on("click", ".twitter-tweet", function (e) {
    e.preventDefault();
}) */