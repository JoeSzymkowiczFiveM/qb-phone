...
function SetupCryptoData(Crypto) {
    ...
    $(".crypto-walletid").html(CryptoData.WalletId + ' ' + '<i class="far fa-copy"></i>');
    ...
    if (CryptoData.History.length > 0) {
        ...
        $.each(CryptoData.History, function(i, change) {
            ...
            var Element = '<div class="cryptotab-course-block">' +
                ...
                '<span class="cryptotab-course-block-title">Rate change</span>' +
...
function RefreshCryptoTransactions(data) {
    //console.log("RefreshCryptoTransactions")
        ...
        if (QB.Phone.Data.PlayerData.money.bank >= Price) {
            $.post('https://qb-phone/BuyCrypto', JSON.stringify({
                ...
                if (CryptoData !== false) {
                    ...
                    QB.Phone.Notifications.Add("fas fa-university", "Bank", "There is &dollar; " + Price + ",- written off!", "#004682", 2500);
                } else {
                    QB.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You do not have enough money..", "#004682", 1500);
                }
            });
        } else {
            QB.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don\'t have enough money..", "#004682", 1500);
        }
    } else {
        QB.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "Fill in everything!", "#004682", 1500);
    }
}); */
        ...
        if (CryptoData.Portfolio >= parseInt(Coins)) {
            $.post('https://qb-phone/SellCrypto', JSON.stringify({
                ...
                if (CryptoData !== false) {
                    ...
                    CloseCryptoPage()
                    QB.Phone.Data.PlayerData.money.bank = parseInt(QB.Phone.Data.PlayerData.money.bank) + parseInt(Price);
                    QB.Phone.Notifications.Add("fas fa-university", "Bank", "There is &dollar; " + Price + ",- credited!", "#004682", 2500);
                } else {
                    QB.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don't have enough Qbits..", "#004682", 1500);
                }
            });
        } else {
            QB.Phone.Notifications.Add("fas fa-chart-pie", "Crypto", "You don\'t have enough Qbits..", "#004682", 1500);
...
$(document).on('click', '#transfer-crypto', function(e) {
    ...
    var Coins = Number($(".crypto-action-page-transfer-crypto-input-coins").val());
                ...
                if (Number.isInteger(Coins)) {
                    $.post('https://qb-phone/TransferCrypto', JSON.stringify({
                        ...
                        if (CryptoData == "notenough") {
                            QB.Phone.Notifications.Add("fab fa-bitcoin", "Crypto", "You do not have enough Qbits..", "#004682", 1500);
                        } else if (CryptoData == "notvalid") {
                            QB.Phone.Notifications.Add("fab fa-bitcoin", "Crypto", "This Wallet ID does not exist!", "#004682", 2500);
                        } else {
                            UpdateCryptoData(CryptoData)
                            CloseCryptoPage()
                            QB.Phone.Notifications.Add("fas fa-university", "Crypto", "You have transferred " + Coins + " to " + WalletId + "!", "#004682", 2500);
                        }
                    });
                } else {
                    QB.Phone.Notifications.Add("fab fa-bitcoin", "Crypto", "Amount must be a whole number..", "#004682", 2500);
                }
            } else {
                QB.Phone.Notifications.Add("fab fa-bitcoin", "Crypto", "You can not transfer to yourself..", "#004682", 2500);
            }
        } else {
            QB.Phone.Notifications.Add("fab fa-bitcoin", "Crypto", "You do not have enough Qbits..", "#004682", 1500);
        }
    } else {
        QB.Phone.Notifications.Add("fab fa-bitcoin", "Crypto", "Fill in everything!", "#004682", 1500);
    }
});
...
$(".crypto-action-page-buy-crypto-input-money").keyup(function() {
    ...
    $(".crypto-action-page-buy-crypto-input-coins").val((MoneyInput / CryptoData.Worth).toFixed(0));
});
...
$(".crypto-action-page-sell-crypto-input-money").keyup(function() {
    ...
    $(".crypto-action-page-sell-crypto-input-coins").val((MoneyInput / CryptoData.Worth).toFixed(0));
});
...
$(document).on('click', '.crypto-walletid', function(event) {
    var textArea = document.createElement('textarea');
    var text = document.getElementById("crypto-account").innerText.trim();
    ...
    QB.Phone.Notifications.Add("fab fa-bitcoin", "Crypto", "Crypto wallet id copied!", "#004682", 1500);
});